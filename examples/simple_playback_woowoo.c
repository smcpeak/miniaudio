/*
This example demonstrates playback of a periodically-varying sine wave
by directly specifying the sample values.  The intention is that a new
miniaudio user who wants to learn the low-level API can start here.

In contrast, simple_playback_sine.c uses the 'ma_waveform' API to
generate samples, which makes it a little bit harder to understand.
